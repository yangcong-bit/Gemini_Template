 ...
 * ========================================== */
typedef struct {
    char type[5];    // 车辆类型 (CNBR / VNBR)
    char id[5];      // 车辆识别码 (4位)
    int y, m, d, h, min, s; // 入库时间
...
typedef struct {
    ...
    CarRecord_t cars[8];  // 8个车位
    
    int cnbr_cnt;         // CNBR车辆数
    int vnbr_cnt;         // VNBR车辆数
    int idle_cnt;         // 空闲车位数 (开机初始化为 8)
    
    float c_rate;         // CNBR费率 (开机初始化为 3.50)
    float v_rate;         // VNBR费率 (开机初始化为 2.00)
    
    uint8_t pa7_mode;     // PA7输出模式：0为低电平，1为2KHz 20%脉冲
